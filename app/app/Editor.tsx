...
// Bouton toolbar — plus grand sur mobile
    ...
    <div className="fixed inset-0 bg-black/20 z-50 flex items-end md:items-center justify-center" onClick={onClose}>
      <div className="bg-white rounded-t-2xl md:rounded-xl shadow-xl p-5 w-full md:w-80" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex gap-2 justify-end">
          <button onClick={onClose} className="px-4 py-2.5 text-sm text-gray-500 hover:text-gray-700">Annuler</button>
          <button onClick={() => onConfirm(url)} className="px-4 py-2.5 text-sm bg-black text-white rounded-lg hover:bg-gray-800">Insérer</button>
        </div>
      </div>
    </div>
  )
...
// Bottom sheet pour les actions tableau sur mobile
      ...
      <div className="w-full bg-white rounded-t-2xl shadow-2xl" onClick={e => e.stopPropagation()}>
        ...
        {actions.map((a, i) => (
          <button
            ...
            className={`w-full text-left px-5 py-4 text-base border-t border-gray-100 transition-colors
              ${a.danger ? 'text-red-500 active:bg-red-50' : 'text-gray-800 active:bg-gray-100'}`}
          >{a.label}</button>
        ))}
...
export default function Editor({ page, pages, onUpdate, onAddSubpage, onNavigate, userId, isMobile }: {
  ...
  const editor = useEditor({
    extensions: [
      ...
      Table.configure({ resizable: !isMobile }),  // resize désactivé sur mobile
  ...
  // Sur mobile : toolbar condensée avec seulement les essentiels
  // Sur desktop : toolbar complète
  const toolbarContent = isMobile ? (
    ...
    <>
      ...
      <ToolBtn
        ...
        active={editor?.isActive('table')} label="⊞ Tableau" title="Insérer un tableau 3×3"
      />
      <Sep />
  ...
  return (
    <div className="flex flex-col flex-1 overflow-hidden">
      ...
      {/* Toolbar — sticky au-dessus du clavier sur mobile */}
      <div
        className="editor-toolbar flex items-center gap-0.5 px-2 border-t border-b border-gray-100 bg-white flex-nowrap overflow-x-auto"
        style={{ minHeight: '48px', position: 'sticky', bottom: 0, zIndex: 10 }}
      >
        {toolbarContent}
      </div>
      ...
      {editor && !isMobile && (
        ...
        >
          <div className="flex items-center gap-0.5 bg-white border border-gray-200 rounded-xl shadow-lg px-2 py-1.5">
            ...
            <button onClick={() => editor.chain().focus().addColumnBefore().run()} className="px-2 py-1 text-xs rounded hover:bg-gray-100 text-gray-600 font-medium">← Col</button>
            <button onClick={() => editor.chain().focus().addColumnAfter().run()} className="px-2 py-1 text-xs rounded hover:bg-gray-100 text-gray-600 font-medium">Col →</button>
            <button onClick={() => editor.chain().focus().addRowBefore().run()} className="px-2 py-1 text-xs rounded hover:bg-gray-100 text-gray-600 font-medium">↑ Ligne</button>
            <button onClick={() => editor.chain().focus().addRowAfter().run()} className="px-2 py-1 text-xs rounded hover:bg-gray-100 text-gray-600 font-medium">Ligne ↓</button>
            <div className="w-px bg-gray-200 self-stretch mx-1" />
            <button onClick={() => editor.chain().focus().deleteColumn().run()} className="px-2 py-1 text-xs rounded hover:bg-red-50 text-red-400 font-medium">− Col</button>
            <button onClick={() => editor.chain().focus().deleteRow().run()} className="px-2 py-1 text-xs rounded hover:bg-red-50 text-red-400 font-medium">− Ligne</button>
            <div className="w-px bg-gray-200 self-stretch mx-1" />
            <button onClick={() => editor.chain().focus().deleteTable().run()} className="px-2 py-1 text-xs rounded hover:bg-red-50 text-red-500 font-medium">🗑</button>
          </div>
        </BubbleMenu>
      )}
      ...
      <div className="flex-1 overflow-y-auto">
        <EditorContent
          editor={editor}
          className="prose max-w-none mx-auto py-6 px-4 md:px-6 md:py-8"
      ...
      <style>{`
        ...
        .ProseMirror table td, .ProseMirror table th {
          min-width: 2em; border: 1px solid #d1d5db; padding: 6px 10px;
          vertical-align: top; box-sizing: border-box; position: relative; font-size: 0.9em;
        }
        ...
        .ProseMirror table .column-resize-handle {
          ...
          width: 4px; background-color: #3b82f6; cursor: col-resize; z-index: 20;
        }
        ...
        @media (max-width: 767px) {
          .ProseMirror { font-size: 16px; line-height: 1.7; }
