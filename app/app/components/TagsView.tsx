...
// ─── TagsInput ────────────────────────────────────────────────────────────────
// compact=true : pas de padding, utilisé dans MetaRow de PageHeader
  ...
  return (
    <div className={compact ? 'relative' : 'relative px-6 pb-3'}>
      ...
      {suggestions.length > 0 && (
        <div
          className="absolute left-0 top-full z-50 rounded-lg shadow-lg overflow-hidden"
...
export function TagsView({ pages, onSelect }: {
  ...
  const allTags: string[] = Array.from(new Set(pages.flatMap(p => p.tags || [] as string[]))).sort()
